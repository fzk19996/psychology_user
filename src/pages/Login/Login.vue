<template>
  <section class="login-container">
    ...
    <div class="register-wrap" v-show="showRegister">
      ...
      <mt-field label="密码" placeholder="请输入密码" v-model="password" type="password"/>
      <mt-field label="确认密码" placeholder="请再次输入密码" type="password" v-model="confirm_password"
                :state="newPswConfirmState" @blur.native.capture="checkNewPswConfirm"/>
      <mt-field label="邮箱" placeholder="请输入邮箱" v-model="email"/>
      <mt-field label="年龄" placeholder="请输入年龄" v-model="age"/>
      <mt-field label="电话" placeholder="请输入电话" v-model="number"/>
...
<script>
  ...
  import {reqLogin, reqRegister, reqFindPsw, userInfo, userRegister} from '../../api'
    ...
    methods: {
      ...
      async checkLogin(){
        ...
        if (result.status == 200) {
          ...
          let user_res = await userInfo();
          console.log(user_res)
          if(user_res.status==200){
            sessionStorage.setItem("userInfo",JSON.stringify(user_res.data)); 
            // 去个人中心界面
      ...
      async register(){
        ...
        if (result.status == 200) {
          MessageBox.confirm('注册成功，是否自动登录?').then(action => {
            //点击确定按钮操作
            this.sno = this.username
            this.stuPsw = this.password
            this.checkLogin();
          },() => {
            //点击取消按钮操作
            this.newSno = '';
